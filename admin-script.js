...
let ordersFilteredOrders = [];    // 存储当前订单列表（全量或状态筛选）
    ...
    function normalizeOrderStatus(status) {
        ...
        if (['pending', '待处理', '待处理中', '待发货', '处理中', 'new', 'processing', 'paid'].includes(raw)) return 'pending';
        if (['hold', '暂缓', '挂起', 'on_hold', 'on-hold', 'paused'].includes(raw)) return 'hold';
        if (['shipped', '已发货', '发货中', '运输中', 'in_transit', 'in-transit'].includes(raw)) return 'shipped';
        if (['arrived', '已到货', '已送达', '待收货', '待签收', 'delivered'].includes(raw)) return 'arrived';
        if (['received_done', 'received', '已收货', '已签收', '签收', 'signed', 'completed', 'done', 'finished', '已完成'].includes(raw)) return 'received';
        if (['cancelled', 'canceled', '已取消', 'cancel', 'closed', '已关闭'].includes(raw)) return 'cancelled';

        console.warn('Unknown order status, fallback to pending:', status);
        return 'pending';
    }
...
function renderOrders(ordersToRender = null) {
    ...
    const selectedStatus = getSelectedOrderStatus();
    const filteredOrders = ordersToRender ?? (selectedStatus === 'all'
        ? orders
